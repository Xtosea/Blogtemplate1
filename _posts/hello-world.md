export const posts = [
  {
    ...
    content: `
Dynamic routing in Next.js allows you to create pages with URLs that depend on data. Each blog post, for example, can have a URL like /posts/post-slug. Static generation improves performance by pre-rendering pages at build time, making your site faster and SEO-friendly.

You can combine dynamic routing with static generation using Next.js’s getStaticPaths and getStaticProps (or the App Router equivalents). This ensures pages are ready to serve without waiting for server requests while still supporting dynamic content.
    `
  },
  {
    slug: "hello-world",
